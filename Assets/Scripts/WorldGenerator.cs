...
public class WorldGenerator : MonoBehaviour
{
    ...
    public IEnumerator CreateNewWorldChunk(Vector3Int _chunkCoord)
    {
        ...
        int[,] _newHeightData = WorldChunkColumns.ContainsKey(_worldColumn) ? WorldChunkColumns[_worldColumn] : null;
        ...
        StartCoroutine(meshCreator.CreateNewMesh(_newChunkData, x => _newChunkMesh = x)); // sets _meshToUse to x when x is returned
